# Importing the libraries needed for the Cleaning of the database
...
# Not all the system will be needing it. So if your system do not need you can delete this part
try:
...
def fixsender(sender, allsenders=None):
    ...
    global dnsmapping
    global mapping
    # Checking for the Sender is None
    if sender is None:
        return None
    # Cleaning the sender email if contains '<' or '>' or ' ' or Capital letter in between
    sender = sender.strip().lower()
    sender = sender.replace('<', '').replace('>', '')

    # Checking if we have hacked gmane.org from address (people hide their address by gmane.org)
...
def parseheader(hdr, allsender=None):
    ...
    # Normalize the domain name of the email addresses
    sender = fixsender(sender, allsender)
...
for message_row in cur_1:
    ...
    if sender_id is None:
        ...
        try:
            ...
            print('Could not retrieved sender id', sender)
            break
    if subject_id is None:
        ...
        try:
            ...
            print('Could not retrieved subject id', subject)
            break
    ...
    try:
        ...
        print('Could not retrieved guid id', guid)
        break
